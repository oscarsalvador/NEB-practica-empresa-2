...
# si usamos el ingress que viene con gitlab
# kubectl rollout restart deployment gitlab-nginx-ingress-controller
# si usamos el ingress de minikube
kubectl rollout restart deployment ingress-nginx-controller -n ingress-nginx

openssl x509 -in gitlab.local.crt -outform PEM > gitlab.gitlab.local.crt
openssl x509 -in ca.crt -outform PEM >> gitlab.gitlab.local.crt
kubectl get secrets gitlab-gitlab-local-cert -o NAME
[ $? = 0 ] && kubectl delete secret gitlab-gitlab-local-cert
kubectl create secret generic gitlab-gitlab-local-cert -n gitlab --from-file=gitlab.gitlab.local.crt
